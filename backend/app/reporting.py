...
def asset_image(doc, path, max_width=155, max_height=90):
    ...
    return InlineImage(doc, str(path), width=Mm(width * factor), height=Mm(height * factor))
...
def pdf_bytes(docx):
    executable = shutil.which(settings.soffice_path)
    if not executable:
        raise HTTPException(503, 'PDF conversion needs LibreOffice. Install it and configure SOFFICE_PATH; DOCX draft export still works.')
    with TemporaryDirectory(prefix='valuer-pdf-') as folder:
        root = Path(folder)
        (root / 'report.docx').write_bytes(docx)
        try:
            result = subprocess.run([executable, '-env:UserInstallation=' + (root / 'profile').as_uri(),
                                     '--headless', '--convert-to', 'pdf', '--outdir', str(root), str(root / 'report.docx')],
                                    capture_output=True, timeout=90, check=False)
        except subprocess.TimeoutExpired:
            raise HTTPException(503, 'PDF conversion timed out. Please try again.')
        output = root / 'report.pdf'
        if result.returncode != 0 or not output.exists():
            raise HTTPException(503, 'PDF conversion failed. Check the server LibreOffice installation and template.')
        return output.read_bytes()


def archive(data, assets, folder):
    folder.mkdir(parents=True)
    docx = docx_bytes(data, 'final', assets)
    pdf = pdf_bytes(docx)
    (folder / 'report.docx').write_bytes(docx)
    (folder / 'report.pdf').write_bytes(pdf)
    manifest = {'template_sha256': hashlib.sha256(TEMPLATE.read_bytes()).hexdigest(),
                'docx_sha256': hashlib.sha256(docx).hexdigest(), 'pdf_sha256': hashlib.sha256(pdf).hexdigest(),
                'data': data.model_dump(mode='json'), 'calculation': calculate(data)}
